...
TEST_CASE("encoder_null out-of-place large buffer", "[encoder_null-oop-01]") {
    ...
    WHEN("empty input") {
        ...
        char buf[bsize];
        std::string src = recode<slip_encoder_hrnull, test_encoder>("");
    ...
    WHEN("null char input") {
        const size_t bsize = 20;
        char buf[bsize];
        std::string src = recode<slip_encoder_hrnull, test_encoder>("0");
    ...
    WHEN("no specials") {
        const size_t bsize = 20;
        char buf[bsize];
        std::string src = recode<slip_encoder_hrnull, test_encoder>("Lorus");
    ...
    WHEN("consecutive specials") {
        const size_t bsize = 20;
        char buf[bsize];
        std::string src = recode<slip_encoder_hrnull, test_encoder>("Lo^#0rus");
    ...
    WHEN("NULL at end") {
        const size_t bsize = 20;
        char buf[bsize];
        std::string src = recode<slip_encoder_hrnull, test_encoder>("Lorus0");
        REQUIRE(6 == src.length());
        REQUIRE( 8 == test_encoder::encoded_size(src.c_str(), src.length()));
        REQUIRE( 8 == (ec_size = test_encoder::encode(buf, bsize, src.c_str(), src.length())));
        REQUIRE("Lorus^@#" == recode<test_encoder, slip_encoder_hrnull>(buf, ec_size));
