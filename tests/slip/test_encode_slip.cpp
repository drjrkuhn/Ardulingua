...
using namespace rdl;
using test_encoder = slip_encoder;
...
TEST_CASE("encoder_slip out-of-place large buffer", "[encoder_slip-oop-01]") {
    ...
    WHEN("empty input") {
        ...
        char buf[bsize];
        sys::StringT src = recode<slip_encoder_hr, test_encoder>("");
    ...
    WHEN("no specials") {
        const size_t bsize = 20;
        char buf[bsize];
        sys::StringT src = recode<slip_encoder_hr, test_encoder>("Lorus");
    ...
    WHEN("consecutive specials") {
        const size_t bsize = 20;
        char buf[bsize];
        sys::StringT src = recode<slip_encoder_hr, test_encoder>("Lo^#rus");
        ...
        /* Standard SLIP: END =\300 ESC =\333 ESCEND =\334 ESCESC =\335 */
        REQUIRE("Lo\333\335\333\334rus\300" == sys::StringT(buf, ec_size));
    }
    ...
    WHEN("consecutive specials at end") {
        ...
        /* Standard SLIP: END =\300 ESC =\333 ESCEND =\334 ESCESC =\335 */
        REQUIRE("Lorus\333\335\333\334\333\334\300" == sys::StringT(buf, ec_size));
    }
}
